...
void Shop::DisplayItems()
{
	if (!availableItems.empty()) {
		...
		for (int i = 0; i < availableItems.size(); ++i) 
		{
			...
			<< availableItems[i]->GetPrice() << "골드" << endl;
		}
	}
}
...
{
	cout << availableItems[index]->GetName() << "을 구매했습니다." << endl;
...
void Shop::SellItem(int index, Inventory* inventory)
{
	if (!inventory->IsInventoryEmpty()) 
	{
		...
		cout << inventory->GetInventoryItem(index)->GetName() << "을 판매했습니다." << endl;
		cout << sellPrice << "골드를 얻어 현재";
		inventory->SetGold(inventory->GetGold()+sellPrice);
		cout << inventory->GetGold() << "골드가 되었습니다." << endl;
